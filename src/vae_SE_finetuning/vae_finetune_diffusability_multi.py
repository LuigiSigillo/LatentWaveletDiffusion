...
# Fix: Import the accelerate module for DistributedDataParallelKwargs
...
def parse_args():
    ...
    parser.add_argument("--multi_gpu", action="store_true", help="Enable explicit multi-GPU training")
    parser.add_argument("--ddp_find_unused_parameters", action="store_true", 
                        help="Set find_unused_parameters=True for DDP (may be needed for some model architectures)")
    parser.add_argument("--sync_batch_stats", action="store_true", 
                        help="Synchronize batch stats across processes during multi-GPU training")
    return parser.parse_args()

def main():
    args = parse_args()
    
    # Set up accelerator with proper configuration for multi-GPU training
    if args.with_tracking:
        accelerator = Accelerator(
            ...
            # DDP specific configurations: Fix - Use DistributedDataParallelKwargs directly
            kwargs_handlers=[
                DistributedDataParallelKwargs(
                    find_unused_parameters=args.ddp_find_unused_parameters,
                    static_graph=not args.ddp_find_unused_parameters  # Enable static graph when possible
                )
            ]
        )
    else:
    ...
    if args.seed is not None:
        set_seed(args.seed)
        # Set seed for each process in distributed training
    ...
    dataloader = DataLoader(
        ...
        num_workers=4,
        pin_memory=True,
        drop_last=True  # Drop last batch to ensure consistent batch size across GPUs
    )
    ...
    # Synchronize batch normalization stats across processes if requested
    if args.sync_batch_stats and accelerator.num_processes > 1:
        vae = torch.nn.SyncBatchNorm.convert_sync_batchnorm(vae)
        logger.info("Synchronized batch normalization layers across processes")
    ...
    # Calculate total batch size (per process)
    total_batch_size = args.batch_size * accelerator.num_processes * args.gradient_accumulation_steps
    logger.info(f"Training on {accelerator.num_processes} GPUs with {args.batch_size} images per GPU "
                f"({total_batch_size} total batch size)")
    logger.info(f"Training for {args.max_train_steps} steps with effective batch size {total_batch_size} "
                f"(total seen images: ~{args.max_train_steps * total_batch_size})")
    ...
    for epoch in range(args.num_train_epochs):
        # Set epoch for the sampler in distributed training
        if accelerator.num_processes > 1:
    ...
    # Save final model - ensure all processes have finished training
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        # Unwrap the model
        ...
        logger.info(f"Training completed after {global_step} steps. Total images seen: {global_step * total_batch_size}")
...
def set_seed(seed):
    ...
    if seed is not None:
        ...
        random.seed(seed)
        np.random.seed(seed)
        # Set seed for data loading operations
        torch.utils.data.random.seed(seed)
